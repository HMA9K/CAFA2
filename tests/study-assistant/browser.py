...
try:
  with sync_playwright() as p:
    ...
    check('Floating assistant stays usable inside an existing exam modal',page.locator('#study-message').is_enabled() and
          page.locator('#study-assistant').evaluate('e=>e.open && !e.matches(":modal")'))
    modal_handle=page.locator('[data-window-move]');modal_handle.focus()
    for _ in range(20):modal_handle.press('Shift+ArrowLeft')
    page.locator('#exam-info-dialog [data-close-info]').first.click()
    page.wait_for_function('document.querySelector("#study-assistant").parentElement===document.body')
    check('Assistant returns to the page when the exam modal closes',page.locator('#study-assistant').is_visible())
    modal_handle.press('Home')
    ...
    page.locator('#calculator-dialog .study-calculator-launch').click()
    check('Desktop assistant floats without making the page inert',page.locator('#study-assistant').evaluate('(e)=>e.open && !e.matches(":modal") && e.getAttribute("aria-modal")==="false"'))
    panel=page.locator('#study-assistant');handle=page.locator('[data-window-move]');resizer=page.locator('[data-window-resize]')
    draft='Dit concept blijft staan tijdens het verplaatsen.'
    page.locator('#study-message').fill(draft);message_count=page.locator('.study-message').count()
    start=panel.bounding_box();grip=handle.bounding_box()
    page.mouse.move(grip['x']+30,grip['y']+20);page.mouse.down()
    page.mouse.move(150,100,steps=12);page.mouse.up()
    moved=panel.bounding_box()
    check('Title-bar drag moves the assistant and keeps it on screen',moved['x']<start['x']-100 and moved['x']>=8 and moved['y']>=8)
    corner=resizer.bounding_box();page.mouse.move(corner['x']+15,corner['y']+15);page.mouse.down()
    page.mouse.move(corner['x']+195,corner['y']+95,steps=12);page.mouse.up()
    enlarged=panel.bounding_box()
    check('Resize corner enlarges the assistant',enlarged['width']>=moved['width']+170 and enlarged['height']>=moved['height']+70)
    handle.focus();handle.press('ArrowRight');keyboard_moved=panel.bounding_box()
    check('Arrow keys move the title bar',abs(keyboard_moved['x']-enlarged['x']-10)<2)
    resizer.focus();resizer.press('Shift+ArrowRight');keyboard_size=panel.bounding_box()
    check('Shift and arrow keys resize the window',abs(keyboard_size['width']-enlarged['width']-40)<2)
    page.locator('[data-window-compact]').click();compact_rect=panel.bounding_box()
    check('Compact button reduces the window',compact_rect['width']<keyboard_size['width'] and compact_rect['height']<keyboard_size['height'])
    page.locator('[data-window-compact]').click();restored=panel.bounding_box()
    check('Compact button restores the chosen size',abs(restored['width']-keyboard_size['width'])<2 and abs(restored['height']-keyboard_size['height'])<2)
    page.locator('[data-window-minimize]').click()
    check('Collapse leaves a reachable title bar',panel.bounding_box()['height']<90 and not page.locator('#study-message').is_visible())
    handle.focus();handle.press('ArrowDown');page.locator('[data-window-minimize]').click()
    check('Collapse and movement preserve the draft and conversation',page.locator('#study-message').input_value()==draft and page.locator('.study-message').count()==message_count)
    page.locator('#calc-expression').fill('4+5');page.locator('#calc-expression').press('Enter')
    check('Calculator stays usable while the assistant floats',panel.evaluate('e=>e.open') and '9' in page.locator('.calc-history-list').inner_text())
    # Focusing either window puts it above the other without a modal backdrop.
    page.locator('#study-message').focus()
    check('Assistant regains the foreground on focus',panel.evaluate('e=>Number(e.style.zIndex)>10010'))
    saved_geometry=panel.bounding_box();close_panel();page.locator('#calculator-dialog .study-calculator-launch').click()
    reopened=panel.bounding_box()
    check('Closing and reopening preserves position and size',all(abs(reopened[k]-saved_geometry[k])<2 for k in ['x','y','width','height']))
    page.screenshot(path=str(OUT/f'desktop-floating-{engine}.png'))
    page.set_viewport_size({'width':390,'height':260});page.wait_for_timeout(100)
    short=panel.bounding_box();compose=page.locator('.study-compose').bounding_box()
    check('Floating window and composer fit a short keyboard viewport',short['x']>=0 and short['y']>=0 and short['x']+short['width']<=390 and
          short['y']+short['height']<=260 and compose['y']+compose['height']<=260)
    page.screenshot(path=str(OUT/f'keyboard-floating-{engine}.png'))
    page.set_viewport_size({'width':1366,'height':950});page.wait_for_timeout(100)
    desktop_again=panel.bounding_box()
    check('Desktop geometry returns after a temporary narrow viewport',all(abs(desktop_again[k]-saved_geometry[k])<2 for k in ['x','y','width','height']))
    page.set_viewport_size({'width':390,'height':260});page.wait_for_timeout(100)
    page.locator('[data-window-minimize]').click();handle.focus()
    for _ in range(8):handle.press('Shift+ArrowDown')
    collapsed_edge=panel.bounding_box()
    check('Collapsed title bar stays within a short viewport at its bottom edge',collapsed_edge['y']+collapsed_edge['height']<=252.5)
    page.set_viewport_size({'width':1366,'height':950});page.locator('[data-window-minimize]').click();page.wait_for_timeout(100)
    grip=handle.bounding_box();page.mouse.move(grip['x']+30,grip['y']+20);page.mouse.down()
    page.mouse.move(saved_geometry['x']+42,saved_geometry['y']+30,steps=6);page.mouse.up()
    saved_geometry=panel.bounding_box()
    page.locator('#study-message').press('Escape')
    check('Escape closes only the assistant and preserves the calculator',not panel.evaluate('e=>e.open') and page.locator('#calculator-dialog').is_visible())
    page.locator('#calculator-dialog .study-calculator-launch').click()
    page.reload();page.wait_for_function('window.StudyAssistant && !document.querySelector(".study-assistant-launch").hidden')
    ensure_open();after_reload=panel.bounding_box()
    check('Window geometry persists across a page reload',all(abs(after_reload[k]-saved_geometry[k])<2 for k in ['x','y','width','height']))
    check('Only geometry is stored in the window preference',set(page.evaluate('JSON.parse(localStorage.getItem("cafa2-assistant-window-v1"))'))=={'size','position','expandedSize'})
    # Reload closes the calculator; reopen to exercise the launcher restoration.
