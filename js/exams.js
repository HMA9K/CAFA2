(function () {
  ...
  function table(rows,completed) {
    return '<div class="exam-table-wrap"><table class="exam-table"><thead><tr>' + (completed?'<th>Toetsnaam</th><th>Ingeleverd</th><th>Beantwoord</th><th>Resultaat</th>':'<th>Toetsnaam</th><th>Beschikbaar</th><th>Duur</th><th>Vragen</th>') + '<th><span class="sr-only">Actie</span></th></tr></thead><tbody>' + rows.join('') + '</tbody></table></div>';
  }
  function dashboard(completed) {
    ...
    var heading = '<div class="exam-page-head"><div><div class="exam-eyebrow">Jouw leeromgeving</div><h1>Dashboard</h1><p>Oefen per onderwerp of maak een volledig CAFA2-tentamen.</p></div><label class="exam-switch">Toetsen <select data-exam-filter aria-label="Toetsen weergeven"><option value="upcoming"' + (!completed?' selected':'') + '>Aankomend</option><option value="completed"' + (completed?' selected':'') + '>Voltooid</option></select></label></div>';
    var html = heading + '<div class="exam-meta-strip"><span><strong>120</strong> MC-oefenvragen</span><span><strong>'+catalog.length+'</strong> volledige tentamens</span><span><strong>'+(completedExams.length+completedPractice.length)+'</strong> voltooide pogingen</span></div>';
    ...
    if (live) html += '<div class="exam-banner"><div><strong>Je tentamen is bezig</strong><p>'+esc(label(live.exam))+' · nog '+Engine.formatTime(Engine.remainingSeconds(live))+'</p></div><a class="btn primary" href="#tentamen/'+encodeURIComponent(live.id)+'">Toets hervatten</a></div>';
    if (completed) {
      var rows = completedExams.map(function (attempt) {
        return {time:attempt.submittedAt,html:'<tr><td><a class="exam-name" href="#inzage/'+encodeURIComponent(attempt.id)+'">'+esc(label(attempt.exam))+'</a><span class="exam-sub">'+(attempt.exam.demo?'Demonstratie · ':'')+(attempt.finishReason==='timeout'?'Tijd verstreken':'Ingeleverd')+'</span></td><td data-label="Ingeleverd">'+datetime(attempt.submittedAt)+'</td><td data-label="Beantwoord">'+Engine.answeredCount(attempt)+' / '+attempt.exam.questions.length+'</td><td data-label="Resultaat">'+(attempt.exam.questions.some(function(q){return q.type==='open';})?'Zelf nakijken':'Bekijk antwoorden')+'</td><td><a class="btn primary" href="#inzage/'+encodeURIComponent(attempt.id)+'">Weergeven</a></td></tr>'};
      }).concat(completedPractice.map(function (item) {
        var target = item.href || '#mc-inzage/' + encodeURIComponent(item.id);
        return {time:item.submittedAt || 0,html:'<tr><td><a class="exam-name" href="'+esc(target)+'">'+esc(item.title)+'</a><span class="exam-sub">MC-oefenvragen · geen tijdslimiet</span></td><td data-label="Ingeleverd">'+datetime(item.submittedAt)+'</td><td data-label="Beantwoord">'+item.answered+' / '+item.total+'</td><td data-label="Resultaat">'+item.good+' / '+item.auto+' nagekeken MC goed</td><td><a class="btn primary" href="'+esc(target)+'">Weergeven</a></td></tr>'};
      })).sort(function(a,b){return new Date(b.time)-new Date(a.time);});
      html += '<section class="exam-section"><h2>Voltooide toetsen</h2>' + (rows.length?table(rows.map(function(r){return r.html;}),true):'<div class="exam-empty"><h3>Nog geen voltooide toetsen</h3><p>Als je een onderwerp of tentamen inlevert, verschijnt je poging hier.</p><a class="btn" href="#dashboard">Naar aankomend</a></div>') + '</section>';
    } else {
      html += '<section class="exam-section"><h2>Oefenvragen</h2>'+table(['<tr><td><a class="exam-name" href="#welkom/practice">CAFA2 oefenvragen</a><span class="exam-sub">Vier onderwerpen · direct nakijken of zelf uitwerken</span></td><td data-label="Beschikbaar"><span class="exam-pill">Altijd beschikbaar</span></td><td data-label="Duur">Geen tijdslimiet</td><td data-label="Vragen">120</td><td><a class="btn primary" href="#welkom/practice">Toets starten</a></td></tr>'],false)+'</section>';
      ...
      function examRow(exam) { return '<tr><td><a class="exam-name" href="#welkom/'+encodeURIComponent(exam.id)+'">'+esc(label(exam))+'</a><span class="exam-sub">Volledig tentamen · '+(exam.sections?exam.sections.length:0)+' opgaven</span></td><td data-label="Beschikbaar">'+(available(exam)?'<span class="exam-pill">Nu beschikbaar</span>':datetime(exam.availableFrom))+'</td><td data-label="Duur">'+exam.durationMinutes+' minuten</td><td data-label="Vragen">'+exam.questions.length+'</td><td><a class="btn primary" href="#welkom/'+encodeURIComponent(exam.id)+'">'+(available(exam)?'Toets starten':'Details bekijken')+'</a></td></tr>'; }
      html += '<section class="exam-section"><h2>CAFA2 tentamens</h2>'+(ready.length?table(ready.map(examRow),false):'<div class="exam-empty"><h3>'+(catalog.length?'Geen nieuwe tentamens':'De tentamenomgeving staat klaar')+'</h3><p>'+(catalog.length?'Je gestarte toets staat hierboven. Ingeleverde toetsen vind je bij Voltooid; vanuit de inzage kun je opnieuw oefenen.':'Hier verschijnen de volledige tentamens zodra ze zijn toegevoegd, met hun tentamendatum en beschikbare tijd.')+'</p>'+(catalog.length?'<a class="btn" href="#dashboard/voltooid">Bekijk voltooide toetsen</a>':'<a class="btn" href="#welkom/demo-omgeving">Probeer de demonstratie</a>')+'</div>')+'</section>';
      ...
      if(soon.length) html += '<section class="exam-section"><h2>Volgende 30 dagen</h2>'+table(soon.map(examRow),false)+'</section>';
    }
    html += '<p class="exam-local-note">Je antwoorden en pogingen blijven in deze browser, op dit apparaat. Ze worden niet naar een docent verstuurd. Bewaar zelf een back-up. Dit is een oefenomgeving, geen beveiligde officiële toetsafname.</p><div class="actions">'+btn('Back-up downloaden','backup')+'<a class="btn" href="#voortgang">MC-voortgang</a></div>';
    host.innerHTML = html;
  }
  ...
  function runner(id) {
    ...
    host.innerHTML='<a class="exam-back" href="#dashboard">‹ Dashboard (de tijd loopt door)</a><div class="frame"><div class="exam-work-head"><div><h1>'+esc(label(attempt.exam))+'</h1><p>'+esc(section?section.title:'Volledig tentamen')+'</p></div><div class="exam-position">VRAAG <strong>'+(i+1)+'</strong> VAN <strong>'+attempt.exam.questions.length+'</strong></div></div><div class="exam-question-body"><div class="exam-question-top"><h2><span class="qnum">'+(i+1)+'</span> '+esc(q.title||'Vraag '+(i+1))+'</h2><span class="small">'+(q.points!==undefined?q.points+' punten':'')+'</span></div><div class="exam-document">'+rich(q.promptHtml,q.prompt)+'</div><span class="exam-answer-label">Vul het antwoord in</span><div data-exam-answer></div><p class="exam-save-status" data-exam-save></p></div><div class="exam-footer"><div class="actions">'+btn('‹ Vorige','previous',false,i===0?'disabled':'')+btn('Volgende ›','next',false,i===attempt.exam.questions.length-1?'disabled':'')+'</div><div class="exam-cirrus-actions">'+btn('Overzicht','overview')+(section?btn('Sectie','section'):'')+btn('Introductie','introduction')+btn(attempt.marked[q.id]?'Gemarkeerd':'Markeren','mark',false,'aria-pressed="'+!!attempt.marked[q.id]+'"')+btn('Toets voltooien','submit',false,'class-placeholder=""')+'</div></div></div>';
  ...
  function overview(attempt) {
    ...
    showModal('Vragenoverzicht','<p>'+Engine.answeredCount(attempt)+' van '+attempt.exam.questions.length+' beantwoord. Een bolletje betekent gemarkeerd.</p>'+groups.map(function(section){return '<h3>'+esc(section.title)+'</h3><div class="exam-progress">'+attempt.exam.questions.map(function(q,i){if((q.sectionId||null)!==section.id)return '';return '<button type="button" data-exam-index="'+i+'" class="'+(answered(attempt,q)?'is-answered ':'')+(attempt.marked[q.id]?'is-marked ':'')+(i===attempt.currentIndex?'is-current':'')+'" aria-label="Vraag '+(i+1)+(answered(attempt,q)?', beantwoord':', niet beantwoord')+'">'+(i+1)+'</button>';}).join('')+'</div>';}).join('')+'<p class="small">Groen = beantwoord. Omlijnd = huidige vraag.</p>');
  }
  function review(id) {
  ...
  function route() {
    ...
    host.hidden=!isExam;document.body.classList.toggle('exam-surface',isExam);document.body.classList.toggle('exam-running',kind==='tentamen');selectedAttempt=kind==='tentamen'?id:null;
